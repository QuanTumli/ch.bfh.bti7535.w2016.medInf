...
public class KFoldHelper {
	...
	public List<TestSet> kFoldFiles(String negativeReviewsPath, String positiveReviewsPath) {
		List<String> documentsPos = FileHelper.getFileNamesFromFolder(negativeReviewsPath);
		...
		for(int i = 0; i < kFoldValue; i++) {
			...
			int endNegFile = sizeOfKFoldNeg + i * sizeOfKFoldNeg - 1;
			int startPosFile = i * sizeOfKFoldPos;
			int endPosFile = sizeOfKFoldPos+ i * sizeOfKFoldPos - 1;
			...
			TestSet newTestSet = new TestSet("Set_Neg_" + startNegFile +"-" + endNegFile + "_Pos_" + startPosFile + "-" + endPosFile);
